...
def run(config:dict):
    ...
    return Run_it({'total_epoch': 500, 'print_it': 1000000, 'total_runs':1, 'decay':config['decay'],\
        'learning_Rate':config['lr'], 'hidden_channels':int(config['hc']),\
        'dropout':config['dropout'], 'layers':int(config['n_lays']), 'x_updates': int(config['x_updates']),\
        'theta_updates': int(config['th_updates']), 'factor': 1, 'x_lr': 1e-06,'th_lr':1e-04,\
        'device': torch.device("cuda" if torch.cuda.is_available() else "cpu"),\
        'batchsize':16, 'total_updates': config['tot_updates'],\
        'name_label':'PROTEINS', 'save_dir':'../Results/proteins/',\
...
if __name__ == "__main__":
    ...
    np.savetxt('results_fi/filename_protein.csv', results)